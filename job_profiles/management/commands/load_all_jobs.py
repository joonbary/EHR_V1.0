        ...
        try:
            with transaction.atomic():
                ...
                for category_name, job_types in job_data.items():
                    # JobCategory 생성
                    category, cat_created = JobCategory.objects.get_or_create(
                        name=category_name,
                        defaults={
                            'code': category_name[:3].upper().replace('/', ''),
                    ...
                    for type_name, roles in job_types.items():
                        # JobType 생성
                        job_type, type_created = JobType.objects.get_or_create(
                            category=category,
                            name=type_name,
                            defaults={
                                'code': type_name[:4].upper().replace('/', ''),
                        ...
                        for role_name in roles:
                            # JobRole 생성
                            job_role, role_created = JobRole.objects.get_or_create(
                                job_type=job_type,
                                name=role_name,
                                defaults={
                                    'code': role_name[:10].upper().replace('/', '').replace(' ', '_'),
                            ...
                            if role_name in job_profiles_data:
                                ...
                                # 기본 프로필 데이터
                                profile_data = {
                                    'role_responsibility': f'{role_name} 관련 주요 업무 수행\n• 업무 기획 및 실행\n• 성과 관리 및 개선\n• 팀 협업 및 커뮤니케이션',
                                    'qualification': f'{role_name} 관련 경험 및 역량\n• 관련 분야 전문성\n• 문제 해결 능력\n• 커뮤니케이션 능력',
                                    'basic_skills': [f'{role_name} 기본 역량', '업무 처리 능력', '협업 능력'],
                                    'applied_skills': [f'{role_name} 전문 역량', '프로젝트 관리', '데이터 분석'],
                                    'related_certifications': ['관련 자격증'],
                                    'growth_path': f'{role_name} → 시니어 {role_name} → 팀장 → 부서장'
                                }
                            
                            # JobProfile 생성 또는 업데이트
                            job_profile, profile_created = JobProfile.objects.update_or_create(
                                job_role=job_role,
                                defaults={
                                    'role_responsibility': profile_data.get('role_responsibility', ''),
                                    'qualification': profile_data.get('qualification', ''),
                                    'basic_skills': profile_data.get('basic_skills', []),
                                    'applied_skills': profile_data.get('applied_skills', []),
                                    'related_certifications': profile_data.get('related_certifications', []),
                                    'growth_path': profile_data.get('growth_path', '')
                                }
                            )
                            
                            if profile_created:
                                self.stdout.write(f'      Created profile for: {role_name}')
                            else:
                                updated_count += 1

                # 통계 출력
                total_roles = sum(len(roles) for job_types in job_data.values() for roles in job_types.values())
                
                self.stdout.write(
                    self.style.SUCCESS(
                        f'\n' + '=' * 50 +
                        f'\nSuccessfully loaded OK Financial Group job profiles!\n' +
                        f'=' * 50 +
                        f'\nTotal job roles expected: {total_roles}\n' +
                        f'New roles created: {created_count}\n' +
                        f'Profiles updated: {updated_count}\n' +
                        '-' * 50 +
                        f'\nDatabase totals:\n' +
                        f'  Categories: {JobCategory.objects.count()}\n' +
                        f'  Types: {JobType.objects.count()}\n' +
                        f'  Roles: {JobRole.objects.count()}\n' +
                        f'  Profiles: {JobProfile.objects.count()}\n' +
                        '=' * 50
                    )
                )
